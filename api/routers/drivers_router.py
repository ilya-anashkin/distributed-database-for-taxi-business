...
async def get_history(id: int):
    ...
    ).join(CarRentsModel, CarRentsModel.id == RidesModel.car_rent_id).filter(
...
async def get_car_info(id: int):
    ...
    ).filter(CarRentsModel.driver_id == id).all()
...
async def get_registered_before(date: str):
    result = session.query(DriversModel).filter(DriversModel.register_date < datetime.datetime.strptime(date, '%Y-%m-%d')).all()
...
async def get_with_status(status: int, employment_status: int):
    result = session.query(
        ...
        DriversStatesModel, DriversStatesModel.driver_id == DriversModel.id
    ).filter(
...
async def get_with_high_rating(rating: int):
    result = session.query(
        DriversModel.taxi_pool_id,
        func.count(DriversModel.id).label('count')
    ).join(
        RidesModel, RidesModel.driver_id == DriversModel.id
    ).filter(
        RidesModel.client_feedback_stars >= rating
    ).group_by(
        DriversModel.taxi_pool_id
    ).all()
